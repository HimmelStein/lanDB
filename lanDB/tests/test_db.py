...
class TestDatabase(unittest.TestCase):
    ...
    def test_database_existancy(self):
        ...
        for dbname in dbnames:
            n = lanDB.check_database(dbname)
            print(dbname, n)
            assert n > 0

    def test_get_max_id_in_table(self):
        id = lanDB.get_max_id_in_table(lanDB.psqlDataBase, "pons")
        print(id)
    ...
    def test_get_ldg_from_pons(self):
        ...
        for lan, id, ref in refLst:
            ldg = lanDB.get_raw_ldg_with_id_from_pons(id, lan=lan)
            print(ldg)
            assert ref == ldg
    ...
    def test_sample_ldg_from_table(self):
        ...
        for id, tb, lan, ref in refLst:
            ldg = lanDB.sample_snt_to_ldg_in_table(id, table=tb, lan=lan)
            print(ldg)
